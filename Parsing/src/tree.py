...
class Tree(object):
    ...
    # Annotate parent
    def annotate_parent(self, Node):
        for child in Node.children:
            if child not in self.root.leaves():
                child.label = (child.label + '^<' + Node.label.split('^')[0] + ">")
            self.annotate_parent(child)

    # Unannotate parent
    def unannotate_parent(self, Node):
        for child in Node.children:
            if child not in self.root.leaves():
                long_label = child.label.split("^")[0]
                child.label = re.sub('[<>]', '', long_label)
            self.unannotate_parent(child)

    # Annotate grand parent
    def annotate_grandparent(self, Node):
        for child in Node.children:
            if child not in self.root.leaves():
                long_label = Node.label.split('^')
                if len(long_label) == 1:
                    child.label = (child.label + '^' + long_label[0] + "")
                if len(long_label) > 1:
                    child.label = (child.label + '^' + long_label[0] + '^' + long_label[1] + "")
            self.annotate_grandparent(child)


    # Unannotate grand parent
    def unannotate_grandparent(self, Node):
        for child in Node.children:
            if child not in self.root.leaves():
